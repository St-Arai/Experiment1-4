...
public class UsersResourcesTest extends TwitterTestBase {
	...
	public void testShowUser() throws Exception {
		...
		// test case for TFJ-91 null pointer exception getting user detail on
		// users with no statuses
		...
		assertNotNull(DataObjectFactory.getRawJSON(user));
		assertEquals(user, DataObjectFactory.createUser(DataObjectFactory
				.getRawJSON(user)));
	...
	public void testLookupUsers() throws TwitterException {
		ResponseList<User> users = twitter1.lookupUsers(new String[] {
				id1.screenName, id2.screenName });
		...
		assertEquals(users.get(0),
				DataObjectFactory.createUser(DataObjectFactory.getRawJSON(users
						.get(0))));
	...
	private void assertContains(ResponseList<User> users, TestUserInfo user) {
		...
		for (User aUser : users) {
			if (aUser.getId() == user.id
					&& aUser.getScreenName().equals(user.screenName)) {
				found = true;
				break;
			}
		}
	...
	public void testAccountMethods() throws Exception {
		...
		assertEquals(original, DataObjectFactory.createUser(DataObjectFactory
				.getRawJSON(original)));
		if (original.getScreenName().endsWith("new")
				|| original.getName().endsWith("new")) {
			original = twitter1.updateProfile("twit4j",
					"http://yusuke.homeip.net/twitter4j/", "location:",
		...
		User altered = twitter1.updateProfile(newName, newURL, newLocation,
				newDescription);
		assertNotNull(DataObjectFactory.getRawJSON(altered));
		assertEquals(original, DataObjectFactory.createUser(DataObjectFactory
				.getRawJSON(original)));
		assertEquals(altered, DataObjectFactory.createUser(DataObjectFactory
				.getRawJSON(altered)));
		twitter1.updateProfile(original.getName(), original.getURL(),
				original.getLocation(), original.getDescription());
		assertEquals(newName, altered.getName());
		assertEquals(newURL, altered.getURL());
		...
		assertEquals(eu,
				DataObjectFactory.createUser(DataObjectFactory.getRawJSON(eu)));
		...
		eu = twitter1.updateProfileColors("87bc44", "9ae4e8", "000000",
				"0000ff", "e0ff92");
		assertNotNull(DataObjectFactory.getRawJSON(eu));
		assertEquals(eu,
				DataObjectFactory.createUser(DataObjectFactory.getRawJSON(eu)));
		...
		AccountSettings intermSettings = twitter1.updateAccountSettings(
				1 /* GLOBAL */, true, "23", "08", "Helsinki", "it");
		...
		AccountSettings lastSettings = twitter1.updateAccountSettings(
				settings.getTrendLocations()[0].getWoeid(),
				settings.isSleepTimeEnabled(), settings.getSleepStartTime(),
				settings.getSleepStartTime(), settings.getTimeZone().getName(),
				settings.getLanguage());
		assertEquals(settings.getLanguage(), lastSettings.getLanguage());
		assertEquals(settings.isSleepTimeEnabled(),
				lastSettings.isSleepTimeEnabled());
		assertEquals(settings.getTimeZone().getName(), lastSettings
				.getTimeZone().getName());
	...
	public void testAccountProfileImageUpdates() throws Exception {
		User user = twitter1.updateProfileImage(new FileInputStream(
				getRandomlyChosenFile()));
		assertNotNull(DataObjectFactory.getRawJSON(user));
		// tile randomly
		User user2 = twitter1.updateProfileBackgroundImage(
				getRandomlyChosenFile(), (5 < System.currentTimeMillis() % 5));
		assertNotNull(DataObjectFactory.getRawJSON(user2));
		assertEquals(user2, DataObjectFactory.createUser(DataObjectFactory
				.getRawJSON(user2)));
	}

	static final String[] profileImages = {
			"src/test/resources/t4j-reverse.jpeg",
			"src/test/resources/t4j-reverse.png",
			"src/test/resources/t4j-reverse.gif",
			"src/test/resources/t4j.jpeg", "src/test/resources/t4j.png",
			"src/test/resources/t4j.gif", };

	static final String[] banners = {
			// gif format fails with
			// {"errors":[{"message":"Image error: is not an accepted format","code":211}]}
	...
	public void testBlockMethods() throws Exception {
		...
		assertEquals(user1, DataObjectFactory.createUser(DataObjectFactory
				.getRawJSON(user1)));
