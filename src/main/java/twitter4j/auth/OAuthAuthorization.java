...
public class OAuthAuthorization implements Authorization, java.io.Serializable, OAuthSupport {
  ...
  public String getAuthorizationHeader(HttpRequest req) {
    return generateAuthorizationHeader(req.getMethod().name(), req.getURL(), req.getParameters(),
        oauthToken);
  }
  ...
  public RequestToken getOAuthRequestToken(String callbackURL, String xAuthAccessType)
    ...
    oauthToken =
        new RequestToken(http.post(conf.getOAuthRequestTokenURL(),
  ...
  public AccessToken getOAuthAccessToken(String oauthVerifier) throws TwitterException {
    ensureTokenIsAvailable();
    oauthToken =
        new AccessToken(http.post(conf.getOAuthAccessTokenURL(),
  ...
  public AccessToken getOAuthAccessToken(String screenName, String password)
    ...
    try {
      ...
      oauthToken =
          new AccessToken(http.post(url, new HttpParameter[] {
  ...
  /* package */String generateAuthorizationHeader(String method, String url,
      HttpParameter[] params, String nonce, String timestamp, OAuthToken otoken) {
    if (null == params) {
    ...
    StringBuilder base =
        new StringBuilder(method).append("&")
   ...
   * @see <a href="http://oauth.net/core/1.0a/#rfc.section.5.4.1">OAuth Core - 5.4.1. Authorization
   *      Header</a>
   */
  /* package */String generateAuthorizationHeader(String method, String url,
    ...
    return generateAuthorizationHeader(method, url, params, String.valueOf(nonce),
        String.valueOf(timestamp), token);
  }
   ...
   * 1. Parameters are sorted by name, using lexicographical byte value ordering. If two or more
   * parameters share the same name, they are sorted by their value. For example:<br>
   * 2. a=1, c=hi%20there, f=25, f=50, f=a, z=p, z=t<br>
   * 3. <br>
   * 4. Parameters are concatenated in their sorted order into a single string. For each parameter,
   * the name is separated from the corresponding value by an ‘=’ character (ASCII code 61), even if
   * the value is empty. Each name-value pair is separated by an ‘&’ character (ASCII code 38). For
   * example:<br>
   ...
   * @see <a href="http://oauth.net/core/1.0#rfc.section.9.1.1">OAuth Core - 9.1.1. Normalize
  ...
  public static String encodeParameters(List<HttpParameter> httpParams, String splitter,
      boolean quot) {
   ...
   * endpoint. The URL used in the Signature Base String MUST include the scheme, authority, and
   * path, and MUST exclude the query and fragment as defined by [RFC3986] section 3.<br>
   * If the absolute request URL is not available to the Service Provider (it is always available to
   * the Consumer), it can be constructed by combining the scheme being used, the HTTP Host header,
   * and the relative HTTP request URL. If the Host header is not available, the Service Provider
   * SHOULD use the host name communicated to the Consumer in the documentation or other means.<br>
   * The Service Provider SHOULD document the form of URL used in the Signature Base String to avoid
   * ambiguity due to URL normalization. Unless specified, URL scheme and authority MUST be
   * lowercase and include the port number; http default port 80 and https default port 443 MUST be
   * excluded.<br>
   * <br>
   ...
   * @see <a href="http://oauth.net/core/1.0#rfc.section.9.1.2">OAuth Core - 9.1.2. Construct
   *      Request URL</a>
   */
